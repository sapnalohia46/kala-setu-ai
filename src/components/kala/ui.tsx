...
export function AppShell({ children, role = "artisan", title, eyebrow, action }: { children: ReactNode; role?: "artisan" | "buyer"; title?: string; eyebrow?: string; action?: ReactNode }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-border bg-card px-5 py-6 lg:flex">
      ...
      <div className="border-t border-border pt-5"><Link to="/" className="flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ChevronLeft className="size-4" />Back to sih 2026</Link><p className="mt-4 font-mono text-[9px] uppercase tracking-[.16em] text-muted-foreground/70">SIH 2026 · Demo mode</p></div>
    </aside>
